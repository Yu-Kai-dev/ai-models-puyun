...
class PuYun(Model):
    ...
    # Download
    download_url = (
        "https://get.ecmwf.int/repository/test-data/ai-models/puyun/{file}"
    )
    download_files = [
        "short",
        "short.onnx",
    ...
    def run(self):
        ...
        with self.stepper(6) as stepper:
            for i in range(total_step):
                ...
                temb = tembs[step_count:step_count+1]
                output, = models[stage].run(
                    None, {'input': input, 'time_embeddings': temb}
                )
                input = np.stack([input[:, -1], output], axis=1)
                pl_chans = len(self.ordering) - len(self.param_sfc)
                pl_data = output[0, :pl_chans]
                sfc_data = output[0, pl_chans:]
